...
//                                  this.getOverridableFile( "settings.xml" ) );
        ...
        throws InterruptedException, HttpException, IOException
    {
        ...
        Assert.assertTrue( (201 == status && shouldUpload) || !shouldUpload, "Artifact upload returned: "+ status + (shouldUpload ? " expected sucess": " expected failure") );
        // if we made it this far we should also test download, because upload implies download
        this.download( repoId, gav, shouldUpload );
    }
        ...
        throws IOException
    {
        ...
        else
        {
            ...
            Assert.assertEquals( 204, statusCode, "Response Status: " + responseText );
